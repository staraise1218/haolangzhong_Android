package com.zhongyi.doctor.utils;
...
public class NetUtils {
    ...
    //参数为要上传的网址，本地照片在本地的地址，我们自己定义的接口
    public static void post_file(final String url, final Map<String, String> map, List<File> files) {
        OkHttpClient client = new OkHttpClient();
        // form 表单形式上传
        MultipartBody.Builder requestBody = new MultipartBody.Builder().setType(MultipartBody.FORM);
        MediaType MutilPart_Form_Data = MediaType.parse("image/*");
        if(files != null&&files.size()>0){

            for (int i = 0; i < files.size(); i++) {
                requestBody.addFormDataPart("file", files.get(i).getName(), RequestBody.create(MutilPart_Form_Data, files.get(i)));
            }
//            // MediaType.parse() 里面是上传的文件类型。
//            RequestBody body = RequestBody.create(MediaType.parse("image/*"), file);
//            String filename = file.getName();
